...
| Field | Current value |
|---|---|
| Overall state | M00-M11 are complete. M12's packaging works and is validated structurally; its disposable-environment install matrix is outstanding, as is all of M13 and M14. `v0.2.0` is published as the latest release and is publicly downloadable (a V2 feature release tracked entirely in `.docs/plans/2026-09-08-v2-usage-trends-and-alerts.md`, not part of this M00-M14 ledger). It is unsigned and predates M14, so R01 stays unchecked |
...
| Next exact action | Run the install matrix in a disposable Windows environment, then close M13's outstanding hardware-dependent checks, including a soak that actually reaches its full duration. `v0.1.2` is public and unsigned, so a defect there is user-visible and must be fixed forward |
...
| v0.1.1 discoverable | 2026-09-08, verified against the GitHub API | The release was published but still could not be found. Cause, confirmed rather than guessed: it was marked prerelease, and `GET /repos/FIEF-nohell/UseNotch/releases/latest` returned 404 because a repository whose only releases are prereleases has no latest release, so GitHub omits the Releases panel from the sidebar. The comparison repository `modshell-cs` differs only in that flag: its `v0.5.2` is `prerelease: false` and shows with a Latest badge. Fixed by promoting `v0.1.1` with `--prerelease=false --latest`; `/releases/latest` now returns `v0.1.1` with the MSI attached. The repository description and eight topics were also set, which is the other difference visible in the sidebar. The workflow and its guard test were changed to require `--latest` and to reject both `--draft` and `--prerelease`. | Tag `v0.1.1` and the published release URL | Discoverable is not verified. The artifact is still unsigned with M12's install matrix unrun, and the release notes say so; R01 remains unchecked |

| Settings overhaul | 2026-09-08, Windows 11 x64 build 26200, 1.75 display scale, verified by screenshotting every section of the running application | Rebuilt the settings window against the layout the plan already specifies: aligned rows with dividers rather than oversized cards. Palette moved to neutral greys (`#0A0A0B` canvas, `#060607` navigation, `#1C1C20` dividers); the blue cast is gone, and colour is now spent only on the checked toggle and the destructive action. Navigation gained a real selected state, a product lockup with the mark, and a one-line description per section, held by a new `SettingsSectionItem` so selection lives in the view model instead of being recomputed in markup. Three defects found and fixed while looking at the running window: Status and Providers rendered identical content, so one navigation entry was pointless; `DataLocation` was declared and never assigned, so About showed an empty row (now supplied through a new `ISettingsRuntime.DescribeDataLocation`); and `SelectedRoot` had no control at all despite being a persisted setting. Long prose overflowed the viewport and the two offset spinners were unlabelled; both corrected. | Screenshots of all six sections under `artifacts/settings-shots` (not committed: they show real credential paths) | Zero-warning Release build, 311 tests, `dotnet format --verify-no-changes`, `git diff --check`. Provider readings showed as Discovering in the captures because the window was photographed 14 seconds after launch, before the first poll completed |

| v0.1.2 candidate | 2026-09-08, Windows 11 x64 build 26200, isolated SDK 8.0.424, plus the hosted windows-2022 runners | Tagged `v0.1.2` on request so the rebuilt settings window ships. The hosted sequence was reproduced locally first: locked restore, zero-warning Release build, 311 solution tests, `dotnet format --verify-no-changes`, `git diff --check`, `Test-VersionAgreement.ps1 -Tag v0.1.2`, `installer/build-installer.ps1` producing `UseNotch-0.1.2-win-x64.msi`, and `installer/Test-Installer.ps1` passing against it, reporting UseNotch 0.1.2, the unchanged upgrade code, and 231 files. Published as the latest release, not a draft and not a prerelease, so it appears in the repository sidebar and answers `/releases/latest`. | Tag `v0.1.2` and the commit it points at | Still unsigned, and M12's install matrix has not been run in a disposable environment. The release notes say both. R01 remains unchecked |

| Overlay trigger reach | 2026-09-08, Windows 11 x64 build 26200, 175% display scale, measured against the running application | Reported as the overlay being hard to summon and needing several attempts. Three compounding causes, each measured before and after rather than reasoned about. First, the edge trigger strip was 6 device-independent pixels, about 10 physical pixels at this scale, so it only caught a pointer thrown hard enough to stop against the screen edge; widened to 28, and a probe stepping the cursor away from the edge confirms the reach went from opening at 10px and failing at 20px, to opening at 40px and failing at 50px. Second, `FarCursorPollInterval` was 400ms and the fast 50ms poll only engages after a poll has already seen the pointer nearby, so first detection could cost most of a second; cut to 150ms. Third, the expand delay is added on top of that, so it went from 120ms to 90ms, while the collapse delay went from 260ms to 420ms because losing the overlay to a twitch costs the whole approach again. Measured open latency over 8 trials from a cold slow poll, cursor placed 5px from the edge: before, min 172ms, avg 463ms, max 516ms; after, min 140ms, avg 172ms, max 281ms. Widening a hover region cannot capture input, and `scripts/Test-OverlayBehavior.ps1` still passes, so transparent space still belongs to the independent process for both the hit test and real input. | Probe output for both builds, produced by stashing the change and rebuilding, so the before figures are measured and not inferred | The CPU cost of the faster poll is NOT established. The first attempt read 3.91% but ran while an unrelated runaway `msedgewebview2.exe` held 15.2GB and 21.8% of the CPU, which also killed a background task for low memory; that reading is void. The re-measurement was blocked because the owner's own installed instance holds the single-instance lock and must not be killed. Idle CPU was already unmet at roughly 2.4% against a target below 0.5%. Re-measure on an idle machine as part of M13 |

| Overlay presentation | 2026-09-08, Windows 11 x64 build 26200, 175% display scale, verified by screenshotting the running overlay with each provider's callout open | Four requested changes. Claude now occupies the top cell and Codex the bottom. The callout sits 8 device-independent pixels clear of the notch instead of touching it. The callout's tail is positioned from code so it points at whichever ring was opened; it was centred before, which only ever looked right for a middle cell that does not exist with two providers. Aiming was corrected once during this change: pointing at the cell put the tail visibly low, because a cell also contains the percentage label, so it aims at the ring. Severity bands changed from a single 80% caution threshold to green below 50%, yellow to 79%, red from 80%, and the ring now takes the same severity colour as the detail bars. That removes the provider accent from the ring deliberately: the same colour previously meant 'this is Claude' in one state and 'this is fine' in another, and the provider mark inside the ring already carries identity. | `artifacts/notch-shots` showing both callouts, the tail aimed at each ring, a 53% reading rendering yellow and a 19% reading rendering green (not committed: they show real quota values) | Zero-warning Release build, 314 tests with the new band boundaries pinned on both sides, `dotnet format --verify-no-changes`, `git diff --check` |

| Ring colour and entrance | 2026-09-08, Windows 11 x64 build 26200, 175% display scale, verified against the running overlay | Two changes. The notch rings return to the provider brand accents at every percentage, reverting the previous entry's decision to colour them by usage; the detail bars keep the green, yellow, red bands. The split is now the rule: a ring answers which provider, a bar answers how close to the limit. A ring drops its accent only when there is no reading, which is not a percentage. Because this rule has now changed twice, it is pinned by tests across all three severities for both providers, plus the unavailable case and the two accents being distinct. Second, the notch slides in from behind the screen edge and fades up over roughly 0.2s instead of appearing at full size. The resting values are applied in a later dispatcher pass, because setting them in the same pass that made the notch visible leaves the transition nothing to animate from. A reduced-motion preference places the notch directly at rest and skips the animation, and a missing transform falls back to the resting state rather than leaving the notch invisible. | Screenshot of the open notch showing Claude's accent at 58% and a grey ring for a provider with no reading (not committed: real quota values) | Zero-warning Release build, 319 tests, `dotnet format --verify-no-changes`, `git diff --check` |

| Overlay stability and mark | 2026-09-08, Windows 11 x64 build 26200, 175% display scale, measured against the running overlay | Three defects, all introduced by the slide-in animation added the release before. First, the overlay opened and collapsed in a loop under a pointer that never moved: the edge strip runs the full window height while the notch is far shorter, so opening dropped the pointer out of the watched set. The strip is now watched in every state, which is the hysteresis the design needed. Measured by holding the pointer still at seven heights for three seconds each: eleven title transitions before, one after, at every height. Second, the notch's cells were not clickable once open. `TranslatePoint` walks the render transform, so during the slide every region was reported up to 120 device-independent pixels off the window, and nothing recomputed regions when a transition ended. Regions are now reported at the resting position by subtracting the live slide offset. Third, a sporadic single collapse remained: `UpdateInteractiveRegions` emptied both caches when `GetClientRect` failed, and empty hover regions mean the pointer is outside. Hover regions are now kept on that failure while interactive regions are still cleared, because clearing those leaves the window click-through, which is the safe direction. The mark was also rebuilt: `New-AppIcon.py` passed only the first frame to the ICO writer, so Pillow resampled the 256 image for every smaller entry and discarded the hand-rendered ones, which is what made it look soft. Each entry is now verified byte-identical to its hand-rendered source. The ring is drawn as a filled annulus sector rather than a stroked arc, and it is now near-white rather than OpenAI's green with a dot in Anthropic's clay: a tool that belongs to neither provider should not wear one provider's colour. | Probe output for both builds, and a per-size comparison of the ICO entries against freshly rendered ones | Zero-warning Release build, 319 tests, `dotnet format --verify-no-changes`, `git diff --check`, `Test-VersionAgreement.ps1 -Tag v0.1.6`, and `scripts/Test-OverlayBehavior.ps1` passing, which is what proves the cells are clickable again while transparent space still belongs to another process |

| v0.2.0 published (V2 feature release) | 2026-09-09, Windows 11 x64 build 26200, isolated SDK 8.0.424, plus the hosted windows-2022 runner | This is not an M00-M14 milestone; it is tracked in full in `.docs/plans/2026-09-08-v2-usage-trends-and-alerts.md`, summarized here only for ledger continuity. Shipped four additions on top of `v0.1.6`: a session-local, in-memory usage trend/burn-rate estimate per quota window (no persistence, resets on restart); user-configurable warning/critical alert thresholds (settings schema bumped to version 2 with a default-filling migration); a transient in-app threshold-crossing notice banner on the overlay, reusing the existing native interactive-region reporting so the no-activate/click-through contract stayed intact (verified interactively with `scripts/Test-OverlayBehavior.ps1`, PASS); and a copy-usage-summary clipboard action in the detail panel that excludes account/credential data. Version bumped to `0.2.0`. Full headless sequence passed (364 tests: 5 Domain, 33 Windows, 75 Provider, 193 Application, 58 UI; zero-warning build; format and whitespace checks clean). `installer/build-installer.ps1` produced `UseNotch-0.2.0-win-x64.msi` (231 files, upgrade code unchanged); `installer/Test-Installer.ps1` passed locally. Tagged `v0.2.0` and pushed; the hosted release workflow built, validated, and published it as the latest release (not draft, not prerelease) in 2m14s. Verified after publishing: `/releases/latest` resolves to `v0.2.0` unauthenticated; the published asset was downloaded independently and its SHA256 (`96a8b63f...`) matched the digest GitHub reports; `Test-Installer.ps1` passed against the downloaded file, reporting UseNotch 0.2.0 and the same 231 files. Release notes were edited post-publish to describe the four new features plainly, stating the trend estimate is session-local and resets on restart. | Tag `v0.2.0`, commit `362d6a1`, and `https://github.com/FIEF-nohell/UseNotch/releases/tag/v0.2.0` | Same open gates as before this release: unsigned artifact, M12's disposable-environment install matrix, M13, M14, and R01 all remain outstanding and are unaffected by this feature release |
...
| 2026-09-08 | The release workflow publishes a real latest release, neither draft nor prerelease | The owner authorized publication. Both hidden states were tried first and both failed the same way. A draft is reachable only by accounts with push access. A prerelease is worse than it looks: with no non-prerelease release the repository has no latest release at all, so `/releases/latest` returns 404 and GitHub hides the Releases panel from the repository sidebar, which is what made the download impossible to find. Limitations belong in the release notes, where people read them, not in a flag that hides the artifact |
