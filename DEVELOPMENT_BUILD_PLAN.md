...
| Field | Current value |
|---|---|
| Overall state | M00-M11 are complete. M12's packaging works and is validated structurally; its disposable-environment install matrix is outstanding, as is all of M13 and M14. A `v0.1.0` candidate tag was pushed on request, which builds an unsigned MSI and opens a draft release. Nothing is published |
| Active milestone | M12's install matrix, then M13 and M14. R01 stays unchecked: the draft release is not a publication |
...
| Next exact action | Run the install matrix in a disposable Windows environment, then close M13's outstanding hardware-dependent checks. Do not publish the `v0.1.0` draft: it is unsigned and M14 has not started |
