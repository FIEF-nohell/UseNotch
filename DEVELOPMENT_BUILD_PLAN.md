...
| Field | Current value |
|---|---|
| Overall state | M06 implementation is verified with synthetic sources and mock HTTP, but its live endpoint compatibility remains an external gate. Work continues with M07 synthetic integration; M03 still awaits a normal-output native smoke after the currently running app exits |
| Active milestone | M07, Anthropic / Claude Code synthetic integration |
| Last completed milestone | M05 |
| Last validated checkpoint | Overlay regression fix `1203d52` (`fix: preserve overlay cells at scaled DPI`); M06 synthetic checkpoint is `929716a`, M05 is `5586660`, M00 is `4d10e83`, M01 is `254447a`, M02 is `c15d86a`, and M04 is `9fd1928`. |
| Branch | `main`, established by the user |
| Worktree | `C:\Users\Noel Hermann\Projects\UseNotch` |
| Files currently changed | M07 Claude source resolution, credential-file reader, OAuth usage adapter, parser fixtures, and this ledger. No provider credentials or raw account data are present |
| Last verification | An isolated Release build of Provider.Tests passed with zero warnings/errors. All 28 provider tests passed, including Claude source precedence, limits-array and named-window merging, deduplication, and an unknown-reset reading. No live credentials or usage endpoints were accessed |
| Next exact action | Add M07 synthetic credential replacement and HTTP failure/header tests, then integrate opt-in Claude polling. Keep M06 unchecked and defer its live endpoint validation to the consolidated Windows validation pass. Do not use or record raw credentials |
...
| Running processes / test environment | User-launched `UseNotch.App` process 35032 is running from normal Release output and was not stopped. Ignored `.tmp/dotnet` contains SDK 8.0.424; `.tmp/m01-clean` contains the clean-source verification build; `TestResults` contains local TRX evidence |
