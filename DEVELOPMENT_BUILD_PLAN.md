...
| Field | Current value |
|---|---|
| Overall state | M07 code for the Codex-mirrored auth/epoch gaps is implemented and covered by synthetic tests, run on a non-Windows machine. M06 and M03 native/live gates are unchanged and still require the Windows environment; nothing Windows-specific was touched |
| Active milestone | M07, Anthropic / Claude Code synthetic integration |
| Last completed milestone | M05 (M06 remains open pending its live endpoint gate; M07 remains open pending live Claude Code inspection) |
...
| Worktree | This session ran on a Linux machine (`/home/nohell/Desktop/UseNotch`), not the Windows worktree above; the user will continue from the Windows machine and expects no Linux-specific changes were made |
| Files currently changed | `CacheAndBackoff.cs` and `StateContracts.cs` (shared `AuthenticationHint` and account-epoch fields, additive-only), `ClaudeUsageProvider.cs` (auth classification, expiry-guidance wording, a credential-retry bug fix), `PollingCoordinatorTests.cs` and `CodexUsageProviderTests.cs` (new Claude and epoch tests), and this ledger. No provider credentials or raw account data are present |
| Last verification | A locally installed .NET 8.0.424 SDK (`~/.dotnet`, outside the repository, no system/package-manager changes) built and ran only the platform-neutral projects (`net8.0`, not `net8.0-windows`): `UseNotch.Domain`, `UseNotch.Application`, `UseNotch.Providers.OpenAI`, `UseNotch.Providers.Anthropic` and their test projects. Release builds were zero-warning/zero-error. `UseNotch.Domain.Tests` 5/5, `UseNotch.Application.Tests` 51/51 (48 prior + 3 new), `UseNotch.Provider.Tests` 41/41 (28 prior + 13 new) passed. `dotnet format --verify-no-changes` passed for each changed project and `git diff --check` passed. `UseNotch.App`, `UseNotch.Platform.Windows`, `UseNotch.UI.Tests`, `UseNotch.Windows.Tests`, and `OverlayInputHarness` target `net8.0-windows` and were not built or run; they require `EnableWindowsTargeting` or an actual Windows host, and no such workaround was added. No native, tray, overlay, or live-credential behavior was exercised |
| Next exact action | On the Windows machine: run the full solution build/test/format sequence from the README to confirm the Linux-side changes also hold there, then inspect a real installed Claude Code CLI's `.credentials.json` and a live `/api/oauth/usage` response to close the last M07 checklist item (targeted versions, source format, compatibility) before checking M07 complete. M06's live vertical slice and M03's native overlay gate remain separately open and unrelated to this session's changes |
| Blocking condition | M03 remains open because this interactive session exposes all monitors at 100%, and per-monitor registry edits did not affect new processes without sign-out. The user requested continuing with M04 before this gate is rerun. This condition was not re-examined in this Linux session |
| Known implementation failures | M06 has no known synthetic-test failure, but the live endpoint returned an unsupported response shape after the known alternate schema support was added. It remains unchecked and deferred, rather than repeatedly retried. The user-launched UseNotch instance did not accept its bounded shutdown signal and continues to lock normal Release output; it was not force-terminated. The overlay harness can lose its foreground-set request when the user changes foreground during an automated run; this is a Windows focus-policy limitation in the harness, not an accepted overlay result. None of this was reproducible or re-checked on Linux |
| Running processes / test environment | This session's Linux-only `~/.dotnet` SDK install and `bin`/`obj` build output under the portable projects are local artifacts, not tracked by git; the previously reported Windows-side running `UseNotch.App` process was not touched from this session |
| Provider data accessed | Repository source and synthetic fixtures only; no live credentials or usage endpoints accessed |
...
- [ ] Record targeted Claude Code versions, source format, supported quota windows, and observed limits of compatibility. Not performed in this session: no Windows host or installed Claude Code CLI was available to inspect a real `.credentials.json` or live usage response. Do the same live inspection this ledger records for M06's `storage.rs` review before checking M07 complete.
