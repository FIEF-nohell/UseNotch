...
- This repo's layer boundaries (Domain has no deps; Application depends only on Domain; provider adapters keep HTTP/credential/parsing details internal) are enforced by tests. Do not add a project reference that crosses them.
